...
def dendro_to_distance(d, mask, points, distances, counter = 0):
    ...
    for s in d.trunk:
        distance_matrix, confidence_matrix, counter = distance_loop(d, s, mask, distance_matrix, confidence_matrix, points, distances, counter)
    return distance_matrix, confidence_matrix, counter

def distance_loop(d, s, mask, distance_matrix, confidence_matrix, points, distances, counter):
    import numpy as np
    ...
    if s.is_branch:
        for c in s.children:
            distance_matrix, confidence_matrix, counter = distance_loop(d, c, mask, distance_matrix, confidence_matrix, points, distances, counter)
    ind_x = s.indices()[0]
    ind_y = s.indices()[1]
