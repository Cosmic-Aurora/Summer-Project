class Cloud:
    ...
    def __init__(self, data, distances, hdr, dx, dy):
        ...
        remove = np.where(np.any((distances[:,0] < min(self.l0,self.l1), distances[:,0] > max(self.l0,self.l1), distances[:,1] < min(self.b0,self.b1), distances[:,1] > max(self.b0,self.b1)), axis = 0))
        pruned_distances = np.delete(distances, remove, axis = 0)
        ...
        final_distances = np.delete(pruned_distances, remove2, axis = 0)
        self.distances = np.hstack((x_values,y_values,final_distances))
