...
remove = np.where(np.any((data[:,2] <=0, data[:,0] < 0, data[:,0] > 40, data[:,2] == 0., np.abs(data[:,1] > 1.16)), axis = 0))[0] #Extracts which data points are useless

useful_data = np.delete(data, remove, axis = 0)
print(len(np.where(useful_data[:,2] == 0)[0]))
